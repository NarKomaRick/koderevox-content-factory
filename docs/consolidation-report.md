...
The configured proxy was not listening, so the original external config was preserved as a backup and Telegram was switched to direct mode. The bot is now healthy and polling Telegram. A real LM Studio chat smoke from the API container returned HTTP 200. The host full suite remains `166 passed`; operations and progress capability smokes also passed after the reboot. Full Producer-to-Director-to-render-to-publish E2E and real publishing remain untested; auto-publish is still disabled.
